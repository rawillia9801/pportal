...
type Puppy = {
  ...
  photos: string[];     // jsonb[] (never null)
  sire_id: string | null;
  dam_id: string | null;
  created_at?: string;
};
...
type PupForm = {
  name: string;
  price: string; // input as text; converted on submit
  status: 'Available' | 'Reserved' | 'Sold';
  gender: 'Male' | 'Female' | '';
  registry: 'AKC' | 'CKC' | 'ACA' | '';
  dob: string;         // YYYY-MM-DD
  ready_date: string;  // YYYY-MM-DD
  photo: string;       // single URL -> wrapped to string[]
  sire_id: string;
  dam_id: string;
};
...
export default function AdminPuppiesPage() {
    ...
    (async () => {
      try {
        const [{ data: pups, error: e1 }, { data: ds, error: e2 }] = await Promise.all([
          ...
          // Avoid assuming a column like `active`; keep it simple and robust
          supabase.from('dogs').select('id,name,sex').order('name', { ascending: true }),
        ]);
        if (e1) throw e1;
        if (e2) throw e2;
  ...
  async function createPuppy(e: React.FormEvent) {
    ...
    setMsg(null);
    try {
      // price: allow empty => null, else parse number
      const priceNum =
        f.price.trim() === '' ? null : Number.isFinite(Number(f.price)) ? Number(f.price) : null;

      // photos: never null (NOT NULL column); allow empty array
      const photos: string[] = f.photo.trim() ? [f.photo.trim()] : [];

      // Normalize blanks -> null for nullable fields
      const gender = (f.gender || null) as 'Male' | 'Female' | null;
      const registry = (f.registry || null) as 'AKC' | 'CKC' | 'ACA' | null;

      // Status is a strict union; matches enum/check constraint
      ...
      // refresh list
      const { data: pups2 } = await supabase
        .from('puppies')
        .select('*')
        ...
        <div style={{ border: '1px solid #eee', borderRadius: 12, padding: 16 }}>
          ...
          <form onSubmit={createPuppy} style={{ display: 'grid', gap: 10 }}>
            ...
            <L label="Status">
              <select
                value={f.status}
                onChange={e => setF({ ...f, status: e.target.value as PupForm['status'] })}
              >
            ...
            <L label="Gender">
              <select
                value={f.gender}
                onChange={e => setF({ ...f, gender: e.target.value as PupForm['gender'] })}
              >
                <option value="">—</option>
                <option>Male</option>
            ...
            <L label="Registry">
              <select
                value={f.registry}
                onChange={e => setF({ ...f, registry: e.target.value as PupForm['registry'] })}
              >
                <option value="">—</option>
                <option>AKC</option>
                <option>CKC</option>
            ...
            <L label="Sire">
              <select
                value={f.sire_id}
                onChange={e => setF({ ...f, sire_id: e.target.value })}
              >
                <option value="">—</option>
                {males.map(m => (
                  ...
                  {rows.map(p => {
                    ...
                    return (
                      <tr key={p.id} style={{ borderTop: '1px solid #f0f0f0' }}>
                        ...
                        <Td>{dam}</Td>
                        <Td>
                          <button onClick={() => r.push(`/admin/puppies/${p.id}`)} style={btnSmall}>
                            Edit
                          </button>
                          <button onClick={() => remove(p.id)} style={btnSmallGhost}>
                            Delete
                          </button>
                        </Td>
                      </tr>
